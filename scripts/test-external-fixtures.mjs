...
if (report.totalCaseCount !== 8) failures.push(`totalCaseCount=${report.totalCaseCount}`);
if (report.totalPassedCount !== 8) failures.push(`totalPassedCount=${report.totalPassedCount}`);
...
if (!gmosRun || gmosRun.caseCount !== 5 || gmosRun.pass !== true) {
  failures.push("curated-gmos run did not pass 5 cases");
}
if (
  !longMemEvalRun ||
