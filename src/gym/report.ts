...
function markdownSlowExternalCases(
  values: ExternalMemoryBenchmarkResult["summary"]["slowestCases"],
): string {
  return values.length
    ? values
        .map(
          (entry) =>
            `${markdownCell(entry.id)}=${entry.durationMs}ms ${entry.pass ? "PASS" : "FAIL"} tokens=${entry.promptTokenEstimate} paths=${entry.reconstructedPathCount}`,
        )
        .join(", ")
    : "none";
}

function markdownSlowExternalGroups(
  values: ExternalMemoryBenchmarkResult["summary"]["slowestCaseGroups"],
): string {
  return values.length
    ? values
        .map(
...
export function renderExternalMemoryBenchmarkMarkdown(
  ...
  return [
    ...
    ...(report.summary.failureSamples.length === 0
      ? ["None"]
      : [
          "| Case | Temporal | Failure reasons | Failure stages | Warnings | Matched expectedAny | Matched expectedAll | Missing expectedAny | Missing expectedAll | Forbidden matches | Missing intent groups | Convergence | Uncertainty | Tokens | Paths | Duration ms | Scoring ms | Taxonomy ms | Wide ms |",
          "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
          ...report.summary.failureSamples.map(
            (entry) =>
              `| ${markdownCell(entry.id)} | ${entry.temporalMode ?? "-"} | ${markdownListCell(entry.failureReasons)} | ${markdownTaxonomyCell(entry.failureTaxonomy ?? [])} | ${markdownListCell(entry.warnings)} | ${markdownListCell(entry.matched.expectedAny)} | ${markdownListCell(entry.matched.expectedAll)} | ${markdownListCell(entry.expectedAnyMissing)} | ${markdownListCell(entry.expectedAllMissing)} | ${markdownListCell(entry.forbiddenMatches)} | ${markdownListCell(entry.missingRequiredIntentGroups)} | ${markdownNullableNumber(entry.evidenceConvergenceScore)}${entry.evidenceConvergenceReached === null ? "" : entry.evidenceConvergenceReached ? " reached" : " not reached"} | ${entry.uncertaintyLevel ?? "-"} | ${entry.promptTokenEstimate} | ${entry.reconstructedPathCount} | ${entry.durationMs} | ${entry.scoringRuntimeMs} | ${entry.taxonomyRuntimeMs} | ${entry.wideBudgetDiagnosticRuntimeMs} |`,
          ),
        ]),
    "",
    "## Cases",
    "",
    "| Case | Status | Mode | Temporal | Failure reasons | Failure stages | Warnings | Matched expectedAny | Matched expectedAll | Missing expectedAny | Missing expectedAll | Forbidden matches | Missing intent groups | Convergence | Uncertainty | Tokens | Paths | Duration ms | Scoring ms | Taxonomy ms | Wide ms |",
    "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...report.cases.map(
      (entry) =>
